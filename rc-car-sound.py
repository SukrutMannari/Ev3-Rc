...
for event in gamepad.read_loop():
    ...
    # --- HANDLE BUTTONS (Sound) ---
    if event.type == ecodes.EV_KEY:
        if event.value == 1:
            if event.code == ecodes.BTN_A:
                speaker.play_tone(300, 0.3, play_type=Sound.PLAY_NO_WAIT_FOR_COMPLETE)
            elif event.code == ecodes.BTN_B:
                song_path = os.path.join(os.path.dirname(__file__), 'cucaracha.wav')
                speaker.play_file(song_path, play_type=Sound.PLAY_NO_WAIT_FOR_COMPLETE)
            elif event.code == ecodes.BTN_X:
                train_path = os.path.join(os.path.dirname(__file__), 'train_horn.wav')
                speaker.play_file(train_path, play_type=Sound.PLAY_NO_WAIT_FOR_COMPLETE)
            elif event.code == ecodes.BTN_Y:
                air_path = os.path.join(os.path.dirname(__file__), 'air_horn.wav')
                speaker.play_file(air_path, play_type=Sound.PLAY_NO_WAIT_FOR_COMPLETE)

    # --- HANDLE JOYSTICKS/TRIGGERS/DPAD (Driving & Lights) ---
